...
async def faq_status(request: Request):
    return await request.app.state.faq_service.status()


@router.post("/faq/refresh")
async def faq_refresh(request: Request):
    try:
        return await request.app.state.faq_service.refresh()
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"FAQ Google Sheets sync failed: {exc}") from exc


@router.get("/public-sources/status")
async def public_sources_status(request: Request):
    context = await request.app.state.fact_service.source_context("en")
    status = request.app.state.public_search.status(
        official_references=context.get("competitor_references", []),
        competitor_facts=context.get("competitor_facts", []),
    )
    status["sheet_b_fetched_at"] = context.get("fetched_at")
    return status

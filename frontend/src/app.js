...
function applyLanguage() {
  ...
  quickReplies.innerHTML = copy().quick
    .map(text => `<button type="button">${escapeHtml(text)}</button>`)
    .join("");
...
function renderConversation() {
  conversation.innerHTML = messages.map(message => {
    ...
    return (
      ...
      + `<div class="message-body">${body}${message.cards.map(renderCard).join("")}</div>`
      + "</article>"
    );
  }).join("");
...
function renderCard(card) {
  ...
  if (card.type === "launch_notification") {
    return (
      '<section class="result-card accent">'
      + `<strong>${escapeHtml(card.product_name || "")}</strong>`
      + `<button class="card-action" data-notify="${escapeHtml(card.target_sku || "")}" data-product="${escapeHtml(card.product_name || "")}">${escapeHtml(copy().notify)}</button>`
      + "</section>"
    );
  }
  ...
  if (card.type === "recommendation") {
    return (
      ...
      + (card.products || []).map(product => {
        ...
        const featureList = (product.features || []).slice(0, 3)
          .map(feature => `<li>${escapeHtml(feature)}</li>`)
          .join("");
        return (
          '<article class="product-card">'
          + `<div class="product-card-head"><strong>${escapeHtml(product.product_name)}</strong>${price}</div>`
          + (featureList ? `<ul>${featureList}</ul>` : "")
          + `<a href="${escapeHtml(href)}" target="_blank" rel="noreferrer">${escapeHtml(productActionLabel(product))}</a>`
          + "</article>"
        );
      }).join("")
      + "</div>"
  ...
  if (card.type === "official_fact") {
    ...
    const specs = card.official_specs_url || card.verified_source_url;
    return (
      '<section class="result-card">'
      + `<div class="source-chip">${escapeHtml(copy().official)}</div>`
      + `<strong>${escapeHtml(card.title || "")}</strong>`
      + `<div class="fact-list">${facts}</div>`
      + (link ? `<a class="inline-link" href="${escapeHtml(link)}" target="_blank" rel="noreferrer">${escapeHtml(copy().view)}</a>` : "")
      + (specs && specs !== link ? `<a class="source-link" href="${escapeHtml(specs)}" target="_blank" rel="noreferrer">Official specs ↗</a>` : "")
      + "</section>"
    );
  }
  ...
  if (card.type === "public_sources") {
    return (
      '<section class="result-card">'
      + `<strong>${escapeHtml(copy().sources)}</strong>`
      + (card.sources || []).map(source => (
        `<a class="source-link" href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)}</a>`
      )).join("")
      + "</section>"
    );
  }
  ...
  while (true) {
    ...
    for (const block of blocks) {
      ...
      try {
        ...
        // Ignore malformed event blocks and continue streaming.
      }
    }
  }
}
...
async function sendMessage(text) {
  ...
  try {
    ...
    await parseSSE(response, (event, data) => {
      ...
      if (event === "card") assistant.cards.push(data);
      if (event === "message") assistant.content += data.delta || "";
      if (event === "error") assistant.content = data.message || copy().error;
      if (event === "done") assistant.pending = false;
      renderConversation();
    });
...
function localizedSlide(slide) {
  if (language === "en") {
    return {
      title: slide.title_en || slide.title,
      subtitle: slide.subtitle_en || slide.subtitle,
      cta: slide.cta_label_en || slide.cta_label
    };
  }
  if (language === "zh") {
    return {
      title: slide.title_zh || slide.title,
      subtitle: slide.subtitle_zh || slide.subtitle,
      cta: slide.cta_label_zh || slide.cta_label
    };
  }
...
function renderHero() {
  ...
  heroStage.innerHTML = (
    `<article class="hero-slide">${media}<div class="hero-overlay"></div>`
    + `<div class="hero-copy"><p class="hero-eyebrow">${escapeHtml(slide.eyebrow || "OPPO")}</p>`
    + `<h2>${escapeHtml(localized.title || "")}</h2>`
    + `<p>${escapeHtml(localized.subtitle || "")}</p>`
    + (localized.cta && slide.cta_url
      ? `<a href="${escapeHtml(slide.cta_url)}">${escapeHtml(localized.cta)} <span>↗</span></a>`
      : "")
    + "</div></article>"
  );
  heroDots.innerHTML = heroSlides.map((_, index) => (
    `<button type="button" data-hero-index="${index}" class="${index === heroIndex ? "active" : ""}" aria-label="Slide ${index + 1}"></button>`
  )).join("");
...
async function submitSubscription(event) {
  ...
  try {
    ...
    postEvent(API, "qualified_private_domain_capture", sessionId, { channel: subscribeChannel });
...
document.querySelector("#hero-carousel").addEventListener("touchstart", event => {
  heroTouchStart = event.touches[0]?.clientX ?? null;
}, { passive: true });
document.querySelector("#hero-carousel").addEventListener("touchend", event => {
  if (heroTouchStart == null) return;
  const end = event.changedTouches[0]?.clientX ?? heroTouchStart;
  if (Math.abs(end - heroTouchStart) > 45) moveHero(end < heroTouchStart ? 1 : -1);
  heroTouchStart = null;
}, { passive: true });
